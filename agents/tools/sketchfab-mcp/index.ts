  ...
  async searchModels(options: {
    ...
    try {
      ...
      const response = await axios.get(
        `${SketchfabApiClient.API_BASE}/search`,
        {
          params,
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        if (status === 401) {
          ...
          throw new Error(
            "Sketchfab API rate limit exceeded. Try again later."
          );
        }
...
server.tool(
  ...
  "Search for 3D models on Sketchfab based on keywords and filters",
  {
    ...
    tags: z
      ...
      .optional()
      .describe(
        'Filter by specific tags (e.g., ["animated", "rigged", "pbr"])'
      ),
    categories: z
      .array(z.string())
      .optional()
      .describe(
        'Filter by categories (e.g., ["characters", "architecture", "vehicles"])'
      ),
    downloadable: z
      .boolean()
      .optional()
...
// Sketchfab Download URLs Tool
server.tool(
  "sketchfab-get-urls",
  "Get download URLs for a 3D model from Sketchfab",
  {
    modelId: z
      .string()
      .describe(
        "The unique ID of the Sketchfab model to get download URLs for (must be downloadable)"
      ),
    format: z
      .enum(["gltf", "glb", "usdz", "source"])
      .optional()
      .describe(
        "Preferred format to get download URL for (defaults to gltf if available)"
      ),
  },
  async ({ modelId, format = "gltf" }) => {
    try {
      // Check if API key is available
      if (!apiKey) {
        return {
      ...
      let downloadUrl = downloadLinks[requestedFormat]?.url;

      if (!downloadUrl) {
        // Use the first available format as fallback
        selectedFormat = availableFormats[0] as keyof typeof downloadLinks;
        downloadUrl = downloadLinks[selectedFormat]!.url;
      }

      // Format all available download links for response
      const formattedLinks = availableFormats
        .map((fmt) => {
          const link = downloadLinks[fmt as keyof typeof downloadLinks]!;
          const expires = new Date(link.expires * 1000).toISOString();
          return `${fmt.toUpperCase()}: ${link.url} (expires: ${expires})`;
        })
        .join("\n");
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting download URLs: ${errorMessage}`,
          },
        ],
      };
    }
...
async function main() {
  // Log API key status to stderr (not stdout which is used for JSON-RPC)
  if (apiKey) {
